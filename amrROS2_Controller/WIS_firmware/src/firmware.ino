...
void control_task(void *arg = nullptr)
{
    ...
    if (emer_state)
    {
        if (!emer_flag)
        {
            ...
            // eMR.cobid = TSDO_COBID + axis2;
            // // CANOpen_ReadActualVelocityObj(&eMR);
            // // current_rpm_left = (float)eMR.ActualVelocity * (3.75 / 16384) * 1.578;
            ...
            // eMR.cobid = TSDO_COBID + axis1;
            // // CANOpen_ReadActualVelocityObj(&eMR);
            // // current_rpm_right = (float)eMR.ActualVelocity * (3.75 / 16384) * 1.578 * (-1);
            // // current_rpm_right = round(current_rpm_right * 100.0) / 100.0;
            
            // CANOpen_ReadActualPosObj(&eMR);
            // float right_pos_m = right_encoder.update((uint32_t)eMR.ActualPosition);

            // Serial5.print("Left wheel [m]: "); Serial5.print(left_pos_m, 4);
            // Serial5.print("  Right wheel [m]: "); Serial5.println(right_pos_m, 4);
    ...
    else
    {
        ...
        // Serial5.print("Right pos: ");
        // Serial5.println(eMR.ActualPosition);
    ...
    last_right_pos_m = right_pos_m;
            ...
            // --- Derive linear & angular motion during dt ---
    float v = (d_left + d_right) / (2.0f * dt);          // m/s forward
    float w = (d_right - d_left) / (WHEELBASE * dt);     // rad/s yaw

    // Serial5.print("Vx: "); Serial5.print(v, 3);
    // Serial5.print("  Wz: "); Serial5.print(w, 3);
    // Serial5.print("d_left: "); Serial5.print(d_left, 4);
    // Serial5.print("  d_right: "); Serial5.print(d_right, 4);
    // Serial5.print("dt: "); Serial5.println(dt, 4);
...
void setup()
{
    ...
    //CAN2.0_TeensyMM_FlexCAN_T4_FIFO_with_interrupts (Program type 2)
