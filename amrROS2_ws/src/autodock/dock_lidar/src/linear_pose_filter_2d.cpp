...
void LinearPoseFilter2D::setCoeff(const std::vector<float>& b,
                                  const std::vector<float>& a) {
  // Check for the smallest vector and use that as the number of elements to
  // push.
  ...
  // Ensure that the sample vectors are appropriately sized. It is done in this
  // way so if we modify the coefficients but want to mainatain history we don't
  // corrupt the past samples.
