...
DockPerception::DockPerception(std::shared_ptr<rclcpp::Node> node_ptr)
 ...
 for (double y = -0.059; y <= 0.059; y += 0.001){
   ...
   // Each side 88 mm-depth
  for (double x = 0.001; x <= 0.088; x += 0.001){
    geometry_msgs::msg::Point p;
    p.x = x;
    p.y = 0.059;
    p.z = 0.0;
  ...
  // Each side 88 mm-depth
  for (double x = 0.088; x > 0.0; x -= 0.001){
    geometry_msgs::msg::Point p;
    p.x = x;
    p.y = 0.121;
    p.z = 0.0;
 ...
 /*for (double y = -0.119; y < 0; y += 0.005) {
  ...
  char str_all[10000] = "";
  for (int i =0; i <ideal_cloud_.size();i++)
  {
    char str_c[800] = "";
  ...
  strcat(str_all,"\n"); 
  RCLCPP_INFO(rclcpp::get_logger("rclcpp"),str_all);
*/
  /*
...
void DockPerception::callback(
  ...
  processor.splitConnected(0.1);// Default 0.04 TODO(enhancement) parameterize
...
*/
  ...
  while (!candidates.empty()) {
    ...
    */
    ...
    index++;
    char str[80] = "";
   // sprintf(str, "candidate %d-%d fit = %f",index,no_candidate,score);
 ...
 /* char str[80] = "";
  ...
  if (fitness >= 0.0) {
    ...
    while (retry-- && (fitness > max_alignment_error_ ||
      ...
      //RCLCPP_INFO(rclcpp::get_logger("rclcpp"),str);
   ...
   //                "width = %f",candidate->width());
    if ((candidate->width() < 0.31)||(candidate->width() >= 0.37)) {
