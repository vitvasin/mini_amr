...
class DeliveryRobotMainController(Node):
    ...
    def localization_status_callback(self, msg: Bool):
        if not msg.data:
            ...
            if self._localization_lost_count >= 60 and self.state != RobotState.LOST:
            ...
            if self.state == RobotState.LOST:
                ...
                if self._localization_recovered_count >= 5:
                    ...
                    self._cancel_lost_monitor_timer()
                    # self.publish_initial_pose() # Disabled so manually corrected poses from RViz are not overwritten
                    self.change_state(RobotState.STANDBY)
  ...
  #      return
    ...
    def on_standby_loop(self):
       ...
       #     return
        ...
        # ถ้า localization ยังไม่พร้อม ให้เปลี่ยนเป็น LOST (รอ 60 วินาทีตามที่ผู้ใช้กำหนด)
        if self._localization_lost_count >= 60:
