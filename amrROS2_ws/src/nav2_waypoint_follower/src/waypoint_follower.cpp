    ...
    "nav2_core::WaypointTaskExecutor")
{
  ...
  declare_parameter("retry_forever", false);
...
WaypointFollower::on_configure(const rclcpp_lifecycle::State & state)
{
  ...
  retry_forever_ = get_parameter("retry_forever").as_bool();
  ...
  const Z & result)
{
  ...
  while (rclcpp::ok()) {
    ...
    if (stop_on_failure_) {
      RCLCPP_WARN(
        get_logger(),
        "Failed to process waypoint %i in waypoint list and stop_on_failure is enabled. "
        "Terminating action.", goal_index);
      action_server->terminate_current(result);
      current_goal_status_.error_code = 0;
      return;

    } else if (retry_forever_) {
      RCLCPP_WARN(
        get_logger(),
        "Failed waypoint %i, retry_forever enabled. Retrying same waypoint...",
        goal_index);

      // Keep goal_index the same and retry
      new_goal = true;
      current_goal_status_.status = ActionStatus::PROCESSING;  // reset
      continue;  // <-- continue while loop, not skip waypoint

    } else {
      RCLCPP_INFO(
        get_logger(),
        "Failed to process waypoint %i, moving to next.", goal_index);
    }
