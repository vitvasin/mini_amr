...
def generate_launch_description():
    ...
    def launch_setup(context, *args, **kwargs):
        use_keepout_zones_str = LaunchConfiguration('use_keepout_zones').perform(context).lower()
        autostart_str = LaunchConfiguration('autostart').perform(context)
        
        # In navigation_amcl.launch.py, the params_file is already pre-processed (RewrittenYaml/ReplaceString)
        # into a temp file, potentially with keepout filter removed if configured there.
        # So we just use the params_file as provided.
                ...
                LoadComposableNodes(
                    ...
                    composable_node_descriptions=[
                        ...
                        ComposableNode(
                            ...
                            name='lifecycle_manager_navigation',
                            parameters=[
                                {'autostart': autostart, 'node_names': lifecycle_nodes_with_keepout if use_keepout_zones_str == 'true' else lifecycle_nodes}
                            ],
                        ),
                    ],
                ),
    ...
    declare_keepout_mask_yaml_cmd = DeclareLaunchArgument(
        'keepout_mask_yaml',
        default_value='/home/smr/Projects/maps/map_keepout.yaml',
        #default_value='',
    ...
    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file',
        # default_value=os.path.join(bringup_dir, 'params', 'nav2_params_keepout.yaml'),
        default_value=os.path.join(navigation_dir, 'config', 'nav2_params_keepout.yaml'),
